...
-(id)initWithFrame:(CGRect)frame
{
    ...
    _font = [UIFont fontWithName:@"Arial" size:22.0f]; 
    _font = [UIFont fontWithName:@"CourierNewPS-BoldMT" size:22.0f];
    _backgroundColor = [[UIColor alloc] initWithRed:.2 green:.2 blue:.8 alpha:0.90];
    _upToMarker = 1.0;
    //NSString* testDialog = @"Dear Elias. You are a faggot. This text can now display very long things so I must inform you of why you are a faget. First of all, ur a fag. Second of all, I need 4 test zones, as 2x2, so I can work on zone collision and shit. Anyways, I got chat working, now what?";
...
{   
    ...
    rect = CGRectMake(0, 350, 320, 110);
    //rect = CGRectOffset(rect, 0, 350);
...
-(void) setNextParagraphofChatDialog
{
    ...
    for(int i = 0; i < count; i++) 
    {
        ...
        if((currentLineChars + nextWordLength) > 21)
        {
            temp = [temp stringByAppendingString:@"\n"];
            currentLineChars = 0;
            linesLeft--;
