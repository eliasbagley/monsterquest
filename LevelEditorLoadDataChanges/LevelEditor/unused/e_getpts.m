function [x,y,button] = e_getpts(varargin)
...
if (nargin < 1)
    ...
    set(GETPTS_FIG, 'CurrentCharacter', 'z');
else
...
elseif (~ishghandle(GETPTS_H1) || ...
    ...
    errStatus = 'ok';
    button = get(GETPTS_FIG, 'CurrentCharacter');
    ...
    if (isempty(x) || button == 'o')
        x = zeros(0,1);
    end
    if (isempty(y) || button == 'o')
        y = zeros(0,1);
    end
end
...
switch key
    ...
    case {char(13), char(3)}   % enter and return keys
        ...
        set(GETPTS_H1, 'UserData', 'Completed');
        %set(GETPTS_FIG, 'CurrentCharacter', 'o'); % so it won't bring up menu when you press return
